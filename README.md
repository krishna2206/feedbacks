...
- **Chat**: channels, private groups, direct messages and threads.
...
> **Status: early development.** Milestone M0 (foundations) is done: auth, organizations and invitations, sync engine, app shell and a working chat channel. See the [roadmap](#roadmap).
...
```bash
pnpm db:seed        # owner demo@example.com / demo-password, organization "Demo"
```
...
| `pnpm test:e2e-sync` | Integration test on an isolated stack: auth, invitations, Zero writes, permissions |
...
Four containers: `postgres` (with `wal_level=logical`), `api`, `zero-cache` and `web`. The `web` container is the single entry point: nginx serves the app and routes `/api` to the API and `/sync` to zero-cache. Behind Traefik, Dokploy or any other reverse proxy, point your domain at `web:80`. Migrations run automatically when the API starts.
...
| M1 | Chat: threads, reactions, attachments (S3), unread state, direct messages |
